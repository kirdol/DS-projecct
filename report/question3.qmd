...
We will analyse the impact of the COVID, natural disasters and conflicts (# deaths, damages, etc.) on the SDG scores. You will find more precise information about how the impact of these events are materialized into our data description section.
...
```{r echo = FALSE, message = FALSE, warning=FALSE}
#### Shapiro test on total_affected, total_deaths and overallscore ####

# Selecting the variables to perform the test
variables_to_test <- data_question3_1[, c("overallscore",
                                          "total_affected",
                                          "total_deaths")]
...
shapiro_results <- data.frame(
  Variable = c("overallscore",
               "total_affected",
               "total_deaths"),
  W_Value = c(shapiro_test_overallscore$statistic,
              shapiro_test_total_affected$statistic,
              shapiro_test_total_deaths$statistic),
  P_Value = c(shapiro_test_overallscore$p.value,
              shapiro_test_total_affected$p.value,
              shapiro_test_total_deaths$p.value))

# printing the result of the shapiro test
print(shapiro_results)
```
The test statistic in the Shapiro-Wilk test is denoted as "W_value." It is a numerical value that summarizes how well the data fits a normal distribution. The value of W suggest whether there is a significant deviation from normality (if low value) or not (if high value). Here they are very close to zero (W = 0.06 and W=0.03) for the disasters variables, this suggests that the data diverge considerably from a normal distribution.

```{r echo = FALSE, message = FALSE, warning=FALSE}
#### Shapiro test on cases_per_million, deaths_per_million and stringency ####

# Selecting the variables to perform the test
variables_to_test <- data_question3_2[, c("cases_per_million",
                                          "deaths_per_million",
                                          "stringency")]
...
shapiro_results_2 <- data.frame(
  Variable = c("cases_per_million",
               "deaths_per_million",
               "stringency"),
  W_Value = c(shapiro_test_cases_per_million$statistic,
              shapiro_test_deaths_per_million$statistic,
              shapiro_test_stringency$statistic),
  P_Value = c(shapiro_test_cases_per_million$p.value,
              shapiro_test_deaths_per_million$p.value,
              shapiro_test_stringency$p.value))
...
```{r echo = FALSE, message = FALSE, warning=FALSE}
#### Shapiro test on ongoing, sum_deaths, pop_affected, area_affected and maxintensity ####

# Selecting the variables to perform the test
variables_to_test <- data_question3_3[, c("ongoing",
                                          "sum_deaths",
                                          "pop_affected",
                                          "area_affected",
                                          "maxintensity")]
...
shapiro_results_3 <- data.frame(
  Variable = c("ongoing",
               ...
               "area_affected",
               "maxintensity"),
  W_Value = c(shapiro_test_ongoing$statistic,
              shapiro_test_sum_deaths$statistic,
              shapiro_test_pop_affected$statistic,
              shapiro_test_area_affected$statistic,
              shapiro_test_maxintensity$statistic),
  P_Value = c(shapiro_test_ongoing$p.value,
              shapiro_test_sum_deaths$p.value,
              shapiro_test_pop_affected$p.value,
              shapiro_test_area_affected$p.value,
              shapiro_test_maxintensity$p.value))

# printing the result of the shapiro test
print(shapiro_results_3)
```
The value of W are close to zero (W = 0.433, W= 0.137, W= 0.327, W= 0.288 and W=0.453) for the conflicts variables, this suggests that the data concerning conflicts diverge from a normal distribution.

To sum this results, it means that for further analysis between these events and the SDG scores we will use other methods, such as Spearman's rank correlation.
...
#### Correlation table and distribution of disasters variables ####
...
To explore our data on events such as disasters, COVID-19 and conflicts we have to first see which regions are the most affected by these. To do so, we made time-series analysis on these three events each time depending on different variables.
...
This is our time-analysis concerning climatic disasters with total affected per region and with total deaths per region.

We can see that the regions that are the most affected by climate disasters are East Asia, North America and South Asia. We will then focus for further analysis on these regions.
    ...
    updatemenus = list(
      list(
        ...
        buttons = list(
          list(method = "restyle",
               args = list("visible",
                           make_visibility_vector(length(combined_plotly$x$data), 1)),
               label = "Total Affected"),
          list(method = "restyle",
               args = list("visible",
                           make_visibility_vector(length(combined_plotly$x$data), 2)),
               label = "Total Deaths")))),
    showlegend = FALSE)

# Show the combined plotly
combined_plotly
```

This is our time-analysis concerning the COVID-19 cases per million, the COVID-19 deaths per million and the COVID-19 stringency by region between end 2018 and 2022. We will keep all the regions as they are all impacted by the COVID-19.
...
combined_ggplot <- ggplot() +
  geom_smooth(data = covid_filtered,
              aes(x = year,
                  y = cases_per_million,
                  group = region), 
              color = "lightblue",
...
This is our time-analysis concerning conflicts deaths, conflicts affected population and maxintensity conflicts per region between 2000 and 2016

We can see that the regions that are the most affected by the deaths are: Middle east and north Africa, Sub-Saharan Africa, South Asia, then very less for America & the Caribbean and Eastern Europe. The most affected population per region are: Middle east and north Africa, Sub-Saharan Africa, South Asia, Latin America & the Caribbean and Caucasus and Central Asia. Finally, the regions having a high maxintensity in conflicts are : Caucasus and Central Asia, Eastern Europe, Middle east and north Africa, Sub-Saharan Africa, North America, South Asia, East Asia just at one precise moment, Latin America & the Caribbean less.
...
combined_ggplot <- ggplot() +
  geom_smooth(data = conflicts_filtered,
              aes(x = year,
                  y = sum_deaths,
                  group = region),
              color = "darkred",
...
Now that we could visualize which regions are the most impacted by these three events we can do correlations analysis per region to see if these events have indeed an impact on the evolution of SDG scores.

### Focus on the correlation between the SDG scores and the different events.

Here you can see an extract containing all the variables that we find important to keep and each SDG score of:

\- Our correlation map between climate disasters and SDG scores in South and East Asia and North America as it was the regions that where the most impacted. We conclude that climate disasters do not really have a big impact on SDG scores per region.

\- Our correlation map between COVID-19 and SDG scores only during COVID-19 time. We have the same conclusions, it is still not significant, and that's quite surprising.

\- Our correlation map between conflicts' deaths and SDG scores in Middle East & North Africa, Sub-Saharan Africa, South Asia, Latin America & the Caribbean as it was the regions that where the most impacted and still do not seem to find a significant relationship.

\- Our correlation map between conflicts affected population and SDG scores only for the Middle East & North Africa, Sub-Saharan Africa, South Asia, Latin America & the Caribbean and Caucasus & Central Asia.

\- Our correlation map between the regions having a maxintensity in conflicts and the SDG scores only for Caucasus & Central Asia, Eastern Europe, Middle east & north Africa, Sub-Saharan Africa, North America, South Asia, East Asia, Latin America & the Caribbean.

```{r, echo = TRUE, message = FALSE, warning = FALSE, fig.height = 2.5}
#### Correlation between the climate disasters and the SDG scores in South and East Asia and North America heatmap ####
...
ggplot(data = cor_melted,
  ...
  labs(x = '',
       y = '',
       title = 'Correlation between the climate disasters and the SDG scores\nin South and East Asia and North America')


#### Correlation between COVID and the SDG scores heatmap ####
...
ggplot(data = cor_melted,
  ...
  labs(x = '',
       y = '',
       title = 'Correlation between COVID and the SDG scores')


#### Correlation between Conflicts Deaths and the SDG scores heatmap ####
...
ggplot(data = cor_melted,
  ...
  labs(x = '',
       y = 'Deaths',
       title = 'Correlation between Conflicts Deaths and the SDG scores')


#### Correlation between Conflicts Affected Population and the SDG scores ####
...
coord_fixed(ratio = 1) +
  labs(x = '',
       ...
       title = 'Correlation between Conflicts Affected Population and the SDG scores')


#### Correlation between Maxintensity in Conflicts and the SDG scores ####
...
ggplot(data = cor_melted,
  ...
  labs(x = '',
       y = 'Maxintensity',
       title = 'Correlation between Maxintensity in Conflicts and the SDG scores')
```
...
Here you can see for example our correlation map between the climate disasters and the SDG scores in South and East Asia and North America with one year gap.

```{r, echo = TRUE, message = FALSE, warning=FALSE, fig.height = 2.5}
#### Correlation between the climate disasters and the SDG scores in South and East Asia with 1 year gap ####
...
ggplot(data = cor_melted_lagged,
  ...
  labs(x = '',
       y = '',
       title = 'Correlation between the climate disasters \nand the SDG scores in South and East Asia with 1 year gap')
```

Even with a year gap it does not seem that climate disasters with such consequences as the population that gets affected and dies has an impact on the SDG scores as we would have thought. But we are still a little bit optimistic and thought why not look at the correlations with a gap year over the years.

### Interactive map of the correlation between the different events and the SDG scores with 1 year gap.

Here you can see an extract of our interactive map of the correlation between the climate disasters and the SDG scores in South, East Asia and North America with 1 year gap. To better understand the results, if we select a specific year (e.g., 2020) in the app, the analysis will show correlations between the SDG scores for the selected year (e.g., 2020) and the disaster-related variables (total_affected and total_deaths) from the previous year (e.g., 2019). Unfortunately, our shiny app is not supported in static R Markdown documents. For more details you can see our interactive maps in the "See Interactive" folder, our document is called "Interactive_Matrix_Plots".

![Extract of the correlation between the climate disasters and the SDG scores with 1 year gap](Q3_Climate_Dis_year_gap.png)

Here you can see the correlation between COVID-19 and the SDG scores with 1 year gap. And strangely, instead of having a negative correlation, we expected that the more cases and deaths happened because of COVID-19, the more the scores of the SDG would be negatively affected, but with the gap year we can see that the scores of Goal3, Goal6, Goal9 and Goal16 are quite positively impacted by COVID-19. Goal6 could be explained by the fact that during COVID-19, the majority of the countries put rules to stay at home, meaning that we stopped contributing to water pollution. However, the fact that the relationship with the others goals is relatively positive is quite odd. In 2022, Goals 12 and 13 ("Responsible Consumption and Production" and "Climate action") are quite negatively impacted by COVID-19.
...
Finally, you can see an extract of our interactive map of the correlation between the 3 different variables of the Conflict dataset and the SDG scores with 1 year gap. Once again, you can look for more details to our document containing our interactive correlation matrices.

![Extract of the correlation between the conflicts deaths and the SDG scores with 1 year gap](Q3_Conflicts_Deaths_year_gap.png)

Here is the extract of our interactive map of the correlation between the affected population in conflicts and the SDG scores with 1 year gap.

![Extract of the correlation between the conflicts affected and the SDG scores with 1 year gap](Q3_Conflicts_Affected_year_gap.png)

Here is the extract of our interactive map of the correlation between the Maxintensity in conflicts and the SDG scores with 1 year gap.

![Extract of the correlation between the conflicts maxintensity and the SDG scores with 1 year gap](Q3_Conflicts_Maxintensity_year_gap.png)The results seems logical, because if the SDG scores continue to go higher and the conflicts remains the same or finishes we get a negative correlation.

Our last idea is to make regressions between the SDG scores and the variables of each event that we thought interesting.

### Regressions between the SDG scores and the events variables.

Let's see the regressions for each score depending on each variable in the disasters dataset (total_affected and total_deaths).

```{r, echo = TRUE, message = FALSE, warning = FALSE, fig.height = 5.7, out.width="100%"}
#### Analysis of Disaster Variables Data per Goals ####
...
subplot_plots <- lapply(plots, function(plot_pair) subplot(plot_pair[[1]],
                                                           ...
                                                           nrows = 1,
                                                           margin = 0.06))
combined_plots <- subplot(subplot_plots, nrows = length(subplot_plots), margin = 0.06)

# Add a main title using the layout function
final_plot <- layout(combined_plots, 
                     title = "Analysis of Disaster Variables Data per Goals<br><sub>Left: Total Affected Variable | Right: Total Deaths Variable</sub>", margin = list(t = 100))
...
Now, we have come to the same conclusions for the COVID-19 and Conflicts events, you can have a nice representation of each regression plot if you look at our Interactive_Matrix_Plots document containing shiny apps, so that you can have a closer look and choose which goals interests you the most.("report/See Interactive/Interactive_Matrix_Plots.qmd")
