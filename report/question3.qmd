...
Q3.1[is.na(Q3.1)] <- 0
ggplot(data = Q3.1, aes(x = year, y = total_deaths, group = region, color = region)) +
  geom_smooth(method = "loess",  se = FALSE, span = 0.7, size = 0.5) + 
...
generate_plot <- function(choice, conflicts_filtered) {
  if (choice == "Deaths by Conflicts") {
    plot <- ggplot(data = conflicts_filtered, aes(x = year, y = sum_deaths, group = region, color = region)) +
      geom_smooth(method = "loess", se = FALSE, span = 0.3, size = 0.6) +
    ...
    plot <- ggplot(data = conflicts_filtered, aes(x = year, y = pop_affected, group = region, color = region)) +
      geom_smooth(method = "loess", se = FALSE, span = 0.3, size = 0.6) +
      labs(x = "Year", y = "Population Affected") +
      facet_wrap(~ region, nrow = 5) +
      theme_minimal() +
    ...
    plot <- ggplot(data = conflicts_filtered, aes(x = year, y = maxintensity, group = region, color = region)) +
      geom_smooth(method = "loess", se = FALSE, span = 0.3, size = 0.6) +
      labs(x = "Year", y = "Maxintensity") +
      facet_wrap(~ region, nrow = 5) +
      theme_minimal() +
...
ggplot(data = cor_melted, aes(Variable1, Variable2, fill = Correlation)) +
  geom_tile(width = 1.05, height = 1.05) +
...
```{r, echo = TRUE, message = FALSE, warning=FALSE, fig.width=6, fig.height=2.55, cache = TRUE, out.width="100%"}
