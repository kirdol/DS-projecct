...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of Total Affected from Climatic Disasters Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of Total Deaths from Climatic Disasters Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of COVID-19 Cases per Million Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of COVID-19 Deaths per Million Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of COVID-19 Stringency Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of Conflicts Deaths Over Time graph ####

# filter data for year over 2000 and lower than 2016
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of Population Affected by Conflicts Over Time graph ####
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#### Trend of Maxintensity by Conflicts Over Time graph ####
...
# Hide the Modebar
initial_plotly <- initial_plotly %>% config(displayModeBar = FALSE)
...
>>>>>>> 9be45b774657ecca6795f220dfc0530dd183c7ad
subset_data <- disaster_data[, relevant_columns]
...
generate_regression_plot <- function(goal) {
  # Your regression plot code here
  # Return a ggplot object
  plot <- ggplot(subset_data, aes_string(x = "total_affected", y = goal)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    labs(title = paste("Regression plot for", goal, "vs Total Affected"))
  
  return(plot)
}

# List of goals
goal_list <- c("goal1", "goal2", "goal3", "goal4", "goal5", "goal6", "goal7", "goal8", "goal9", "goal10", "goal11", "goal12", "goal13", "goal15", "goal16")

# Create buttons for interactive plot
buttons <- lapply(goal_list, function(goal) {
  list(
    method = "restyle",
    args = list('transforms[0].target', goal),
    label = goal
  )
})

# Initial plot
p <- generate_regression_plot(goal_list[1])
p <- ggplotly(p)

# Add transform for interactivity
p <- p %>% layout(
  updatemenus = list(
    list(
      type = "dropdown",
...
for (goal in relevant_columns[1:15]) {
  # Pour total_affected
  p_affected <- ggplot(subset_data, aes_string(x = "total_affected", y = goal)) +
    geom_point(size = 0.1) +
    geom_smooth(method = "lm", formula = y ~ x, se = FALSE, size = 0.5) +
    labs(x = "Total Affected", y = paste("Goal", substr(goal, 5, 6)))

  # Pour total_deaths
  p_deaths <- ggplot(subset_data, aes_string(x = "total_deaths", y = goal)) +
    geom_point(size = 0.1) +
    geom_smooth(method = "lm", formula = y ~ x, se = FALSE, size = 0.5) +
    labs(x = "Total Deaths", y = paste("Goal", substr(goal, 5, 6)))

  # Ajouter les deux graphiques à la liste
  plots[[goal]] <- list(ggplotly(p_affected), ggplotly(p_deaths))
}

# Afficher tous les graphiques
subplot_plots <- lapply(plots, function(plot_pair) subplot(plot_pair[[1]], plot_pair[[2]], nrows = 1, margin = 0.05))
subplot(subplot_plots, nrows = length(subplot_plots), margin = 0.05)
