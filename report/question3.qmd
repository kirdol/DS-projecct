...
shapiro_test_overallscore
shapiro_test_total_affected
shapiro_test_total_deaths
...
shapiro_test_cases_per_million
shapiro_test_deaths_per_million
shapiro_test_stringency
```
...
shapiro_results <- lapply(variables_to_test, shapiro.test)

names(shapiro_results) <- c("cases_per_million", "deaths_per_million", "stringency")
shapiro_results
```
The value of W are close to zero (W = 0.2, W= 0.2 and W=0.4) for the conflicts variables, this suggests that the data diverge from a normal distribution.
...
```
We can see that the regions the most affected by the climate disasters are East Asia, North America and South Asia. We will concentrate more in this regions. 

These is our time-analysis concerning climatic disasters with total deaths per region
...
```
We can see that the regions the most affected by the climate disasters are East Asia, Latin America & the Caribbean and South Asia.
...
We can see that the regions the most affected by the deaths are : Middle east and north Africa, Sub-Saharan Africa, South Asia, then very less America & the Caribbean and Eastern Europe.
...
We can see that the regions the most affected by the affected population are : Middle east and north Africa, Sub-Saharan Africa, South Asia, Latin America & the Caribbean and Caucasus and Central Asia.

Finally, these is our time-analysis concerning maxintensity conflicts per region between 2000 and 2016.
```{r, echo = TRUE, message = FALSE, warning = FALSE, cache = TRUE, out.height="100%", out.width="100%"}
...
Here you can see an extract of our correlation map between the climate disasters and the SDG goals in South and East Asia and North America as it was the regions that where the most impacted.
We conclude that climate disasters do not really have a big impact on SDG scores per region.
...
```
Here you can see an extract of our correlation map between the COVID-19 and the SDG goals only during COVID-19 time.
We have the same conclusions, it is still not significant, and that's surprising. 
