...
### D3_2_Military_expenditure_percent_gov_exp {.unnumbered}

```{r}
D3_2_Military_expenditure_percent_gov_exp <- data.frame(
  Column1 = c("code",
              "country",
              "year",
              "MilitaryExpenditurePercentGovExp"),
  Column2 = c("Country code (ISO)", 
              "Country name",
              "Year of the observation (2000-2022)",
              "Military expenditures in percentage of government expenditures"))

colnames(D3_2_Military_expenditure_percent_gov_exp) <- c("Variable Name",
                                   "Explanation")
...
D1_0_SDG <- as.data.frame(read.csv(here("scripts","data","SDG.csv"), sep = ";"))
...
=======
For this first dataset, we went from 4'140 observations for 120 variables to `r nrow(D1_0_SDG)` observations for `r ncol(D1_0_SDG)` variables.
>>>>>>> Stashed changes
...
=======
For this dataset, we went from 82'800 observations for 8 variables to `r nrow(D2_1_Unemployment_rate)` observations for `r ncol(D2_1_Unemployment_rate)` variables.
...
=======
For this dataset, we went from ??? observations for 68 variables to `r nrow(D3_1_GDP_per_capita)` observations for `r ncol(D3_1_GDP_per_capita)` variables.
>>>>>>> Stashed changes
...
For this dataset, we went from ??? observations for 68 variables to 3818 observations for 4 varibles.
=======
For this dataset, we went from ??? observations for 68 variables to `r nrow(D3_2_Military_Expenditure_Percent_GDP)` observations for `r ncol(D3_2_Military_Expenditure_Percent_GDP)` variables.
>>>>>>> Stashed changes
...
datatable(D3_3_Miliraty_Expenditure_Percent_Gov_Exp, 
          ...
          rownames = FALSE)
```
For this dataset, we went from ??? observations for 68 variables to `r nrow(D3_3_Miliraty_Expenditure_Percent_Gov_Exp)` observations for `r ncol(D3_3_Miliraty_Expenditure_Percent_Gov_Exp)` variables.
...
=======
For this dataset, we went from 6'570 observations for 4 variables to `r nrow(D4_0_Internet_usage)` observations for `r ncol(D4_0_Internet_usage)` variables.
>>>>>>> Stashed changes
...
=======
For this dataset, we went from 3'465 observations for 141 variables to `r nrow(D5_0_Human_freedom_index)` observations for `r ncol(D5_0_Human_freedom_index)` variables.
>>>>>>> Stashed changes
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
Conflicts <- as.data.frame(read.csv(here("scripts","data","Conflicts.csv")))
