...
Now, there are no missing values in the 'population' variable, and we observe that it contains information on `length(unique(D1_0_SDG$country))` countries.
...
For this first dataset, we reduced the size from 82'800 observations across 8 variables to 3812 observations for 5 variables.
...
For this first dataset, we reduced the size from 6'570 observations across 4 variables to 3433 observations for 4 variables.
...
For this first dataset, we reduced the size from 3'465 observations across 141 variables to `lengh(D5_0_Human_freedom_index$code)` observations for 4 variables.
