...
**Focus on relationship between SDGs**: How are the different SDGs linked?
...
As we know from the nature of the overallscore variable. It has been calculated using the 17 goal scores. Hence, the stepwise regression did not struggle to find a model with good results. The $R^2\:adjusted$ of 0.98 of this model suggest that the model is good quality. As we can see in the first graph, the residuals vs. fitted plot also suggest a good quality. The residuals are well distributed around 0, the residual and the red line, representing the mean residual line is almost flat and very close to 0. Let's now look at the model that according to our stepwise regression, is the best model at explaining our goal 9 scores with a $R^2\:adjusted$ of 0.81 but in reality does not perform well according to the residual versus fitted plot. The predicted model looks as follow:
