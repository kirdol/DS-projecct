...
For this first part of our EDA, let's try to check first the distribution of the SDG. The following variables are ordered by decreasing order of the average of the scores.
...
ggplot(long_df_goal_distribution, aes(x = Value,
                                      y = Goal,
                                      fill = stat(x))) +
   ...
   output$distPlot <- renderPlot({
      if (currentPlot() == "goal") {
         ggplot(long_df_goal_distribution, aes(x = Value, y = Goal, fill = stat(x))) +
            ...
            labs(x = 'Scores', y = 'Goals', title = 'SDG scores Distribution')
...
All these variables have a right-skewed distribution. Taking the mode into account, most of the concerned countries in our data have an unemployment rate between 2 to 7%, a distribution of GDP per capita between $3'000-\$10'000, a distribution of military expenditure in percentage of the GDP 10% to 20% and finally a internet usage between 0 and 10%.
...
medians_all$color <-
  ifelse(medians_all$median_value > 75, "1",
         ifelse(medians_all$median_value < 25, "3", '2')) # assigning the
# Median core category. If median for a goal is > 75 -> 1,
#                       If < 25 -> red,
#                       orange otherwise.
...
medians_all_HFI$color <- ifelse(medians_all_HFI$median_value > 7.5, "3", 
                        ifelse(medians_all_HFI$median_value < 2.5, "2", '1'))
...
According to the direction of the factors related to the dimension 1, the latter might be showing a fundamental contrast, such as between more regulated versus less regulated environments or between environments with more versus less government intervention. On another hand, dimension 2 might differentiate between aspects of freedoms, such as the rule of law versus personal liberties, or perhaps different types of regulation.
...
Let's find the best model for each goal that does not involve severe multicollinearity (VIF > 5) and that has a variance well explained by our independent variable (a good adjusted Rsquared).
...
plot(reg_goal1,scale="r2") + theme_minimal() + title("Stepwise Regression Goal1 Forward method, adj.R^2")
plot(reg_goal2,scale="r2") + theme_minimal() + title("Stepwise Regression Goal2 Forward method, adj.R^2")
plot(reg_goal3,scale="r2") + theme_minimal() + title("Stepwise Regression Goal3 Forward method, adj.R^2")
plot(reg_goal4,scale="r2") + theme_minimal() + title("Stepwise Regression Goal4 Forward method, adj.R^2")
plot(reg_goal5,scale="r2") + theme_minimal() + title("Stepwise Regression Goal5 Forward method, adj.R^2")
plot(reg_goal6,scale="r2") + theme_minimal() + title("Stepwise Regression Goal6 Forward method, adj.R^2")
plot(reg_goal7,scale="r2") + theme_minimal() + title("Stepwise Regression Goal7 Forward method, adj.R^2")
plot(reg_goal8,scale="r2") + theme_minimal() + title("Stepwise Regression Goal8 Forward method, adj.R^2")
plot(reg_goal9,scale="r2") + theme_minimal() + title("Stepwise Regression Goal9 Forward method, adj.R^2")
plot(reg_goal10,scale="r2") + theme_minimal() + title("Stepwise Regression Goal10 Forward method, adj.R^2")
plot(reg_goal11,scale="r2") + theme_minimal() + title("Stepwise Regression Goal11 Forward method, adj.R^2")
plot(reg_goal12,scale="r2") + theme_minimal() + title("Stepwise Regression Goal12 Forward method, adj.R^2")
plot(reg_goal13,scale="r2") + theme_minimal() + title("Stepwise Regression Goal13 Forward method, adj.R^2")
plot(reg_goal15,scale="r2") + theme_minimal() + title("Stepwise Regression Goal15 Forward method, adj.R^2")
plot(reg_goal16,scale="r2") + theme_minimal() + title("Stepwise Regression Goal16 Forward method, adj.R^2")
plot(reg_goal17,scale="r2") + theme_minimal() + title("Stepwise Regression Goal17 Forward method, adj.R^2")
```
...
In conclusion, after reviewing which variables are correlating between eachother, after taking care of our multicollinearity problems and doing our regressions on our overall SDG score, we have been finally able to get a broad picture of the influence of our factors over our SDG.
