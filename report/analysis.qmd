...
In order to answer the first question of our work, let's start by zooming on the correlation matrix heatmap made in our EDA part. Here are the correlations between the SDG goals and all the other variables except the SDG goals. For visibility purpose, we only included the significant p values superior to 0.75 in absolut value.
...
Now let's zoom on the correlations between all our variables except the SDG goals. Again, for visibility purpose, we only included the significant p values superior to 0.75 in absolut value.
...
fviz_eig(myPCA_s,
  ...
  theme(plot.title = element_text(hjust = 0.5), # Center the title
        plot.title.position = "plot") +
  labs(x = "Number of clusters", y = "Percentage of explained variance",title = "Scree plot")
summary(myPCA_s)
...
Now concerning the Human Freedom Index scores, most of the variables are positively correlated to the dimension 1, slightly less for the PF religion, and finally the EF government variable is slighlty uncorrelated to the dimension 1. With a eigenvalue bigger than 1 for the three first components, we conclude that there are 3 dimensions to take into account. Nevertheless, again, they are explaining less than 80% of accumulated variance. Therefore, the rule of thumb would suggest us to take 4 dimensions into account. 
...
data_kmean_country <- data_question1 %>% dplyr::select(-c(X,code,year,continent,region, population)) #only select numerical observations
...
Now let's continue our cluster analysis.
...
plot(leaps,scale="r2") + 
      theme_minimal() +
      title("Stepwise Regression: adjusted R^2")
```
The model found is taking into account the following dependent variables: unmeployment rate, military expenditure percentage of GDP, internet_usage, pf_security, pf_religion, pf_identity, ef_legal, ef_trade. We notice here that the previous variables highly correlated to the SDG goals (GDP per capita, pf_law, internet_usage and ef_legal), we dropped the first two ones. 
