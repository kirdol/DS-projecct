...
```{r, echo = TRUE, message = FALSE, cache = TRUE, warning=FALSE, out.height="100%", out.width="100%"}
ggplot(data = data4) +
  geom_line(mapping = aes(x = year, y = mean_value, color=goal), size = 0.4) +
  geom_point(mapping = aes(x = year, y = mean_value, color=goal), size = 0.7) +
  scale_color_manual(values = color_palette) +
  scale_y_continuous(limits = c(0, 100)) +
  labs(title = "Evolution of the mean SDG achievement scores across the world",
       y = "Mean SDG Scores",
...
We create one new variable per goal that captures the difference in SDG score between the year of the observation and the previous year. In addition, preparing for the specific question around 2015, we only keep the years from 2009 to 2022 (7 years before and after 2015). 
...
We look at the top 5 countries in terms of improvement each year and we see that 
...
We look at the worst 5 countries in terms of improvement each year and we see that 
...
Did the best improvements happen before or after 2015? Is it different considering the different goals? And which countries are the best improvers?
