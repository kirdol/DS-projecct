...
```{r, echo = TRUE, message = FALSE, warning=FALSE, cache = TRUE, out.height="50%"}
data1 <- data_question2 %>% group_by(year, continent) %>%
  mutate(mean_overall_score_by_year=mean(overallscore))

plot1 <- ggplot(data1) +
  geom_line(mapping=aes(x=year, y=mean_overall_score_by_year, color=continent), lwd=0.5) +
  geom_point(mapping=aes(x=year, y=mean_overall_score_by_year, color=continent), lwd=0.5) +
...
plot4 <- ggplot(data4) +
  ...
  scale_y_continuous(limits = c(-2.1,1.75)) +
  labs(title = "Score difference",
       y = "Mean Overall SDG Score difference",
       x = "Year"
       ) +
...
```{r, echo = TRUE, message = FALSE, cache = TRUE, warning = FALSE, out.height="200%"}
...
plot5 <- ggplot(data = data5) +
  ...
  theme(legend.position="right", legend.title = element_blank(), plot.title = element_text(size= 10), axis.title.x = element_text(size= 8), axis.title.y = element_text(size= 8), legend.text = element_text(size= 6))+
...
plot6 <- ggplot(data = data6) +
  geom_line(mapping = aes(x = year, y = mean_diff_value, color = goal), size = 0.5) +
