...
```{r, echo = FALSE, message = FALSE, warning=FALSE, cache = TRUE, out.height="50%"}
...
plot3 <- ggplot(data3) +
  geom_line(mapping=aes(x=year, y=mean_overall_score_by_year, color=region), lwd=0.5) +
  #geom_point(mapping=aes(x=year, y=mean_overall_score_by_year, color=region), lwd=0.7) +
  scale_y_continuous(limits = c(0, 100)) +
  labs(title = "Evolution of the mean \noverall SDG achievement score",
       y = "Mean Overall SDG Score",
       x = "Year"
       ) +
  theme(legend.position="none",plot.title = element_text(size= 10), axis.title.x = element_text(size= 8), axis.title.y = element_text(size= 8), legend.text = element_text(size= 8),legend.title = element_text(size= 10))

data4 <- data_question2 %>% group_by(year, region) %>%
  mutate(mean_diff_overall_score_by_year=mean(diff_overallscore))

plot4 <- ggplot(data4) +
  geom_line(mapping=aes(x=year, y=mean_diff_overall_score_by_year, color=region), lwd=0.5) +
  #geom_point(mapping=aes(x=year, y=mean_diff_overall_score_by_year, color=region), lwd=0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  scale_y_continuous(limits = c(-0.3,1.2)) +
  labs(title = "Score difference",
       y = "Mean Overall SDG Score difference",
...
```{r, echo = FALSE, message = FALSE, cache = TRUE, warning = FALSE, out.height="200%"}
...
plot6 <- ggplot(data = data6) +
  ...
  theme(legend.position="none", plot.title = element_text(size= 10), axis.title.x = element_text(size= 8), axis.title.y = element_text(size= 8), legend.text = element_text(size= 6),legend.title = element_text(size= 10))
...
```{r, echo = FALSE, message = FALSE, cache = TRUE, warning=FALSE, out.height="100%", out.width="100%"}
data5 <- data_question2 %>%
...
ggplot(data = data5) +
  ...
  theme_light() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6), plot.title = element_text(size=10))
```
...
for (year in unique_years) {
  ...
  plot <- ggplot(data = top_countries, mapping = aes(x = country, y = diff_overallscore, fill = region)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size= 6), axis.text.y = element_text(size= 6), legend.position = "none", plot.title = element_text(size = 10)) + 
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, out.width="100%", out.height="75%"}
# Best improvements
...
Regarding the impact of the adoption of SDGs in 2015, we can not tell that it had a positive impact, because there are not more big improvements after 2015 than before, even a little bit less. In addition, the most impressive improvements mostly occurred before 2015. These conclusions are supported by the next graph: we fit two different regression lines (before and after 2015) to see if there is a jump after the adoption of the SDGs and if the the SDG scores increase faster.
