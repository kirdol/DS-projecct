...
The following boxplot is concerning the SDG goal scores. On the graph we can see that there are a lot of extreme values for the goal 2 (), 4() and 13(), which could show inequalities within certain countries. 
```{r, echo = TRUE, message = FALSE, warning=FALSE, cache = TRUE}
#### boxplots ####
...
Here, by looking at the SDGs individually, we notice that all SDGs except goal 9 (industry innovation and infrastructure) are close to one another in terms of level and growth. Goal 9 starts far below the others in 2000 and growths faster until almost exceeding 50%.
...
Now, comparing the SDG scores by continent, we observe that most of the time, Europe is at the top of the graph and Africa at the bottom, except for goals 12 and 13 that are linked to ecology. Some other information stand out:
