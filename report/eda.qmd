...
# corr_matrix <- matrix(nrow = ncol(sdg_scores2), ncol = ncol(sdg_scores2))
# p_matrix <- matrix(nrow = ncol(sdg_scores2), ncol = ncol(sdg_scores2))
# for (i in 1:ncol(sdg_scores2)) {
#   for (j in 1:ncol(sdg_scores2)) {
#     test_result <- cor.test(sdg_scores2[, i], sdg_scores2[, j])
...
# melted_cor_matrix_Q1 <-
#   melt(corr_matrix)
# melted_p_matrix_Q1 <-
#   melt(matrix(as.vector(p_matrix), nrow = ncol(sdg_scores2)))
# 
# plot_data2 <- # Combine the datasets
#   cbind(melted_cor_matrix_Q1, p_value = melted_p_matrix_Q1$value)
# 
# ggplot(plot_data2, aes(Var1, Var2, fill = value)) +
#   geom_tile() +
#   geom_text(aes(label = sprintf("%.2f", value), color = p_value < 0.05),
#             vjust = 1) +
#   scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
