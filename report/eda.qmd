...
pairs(data_question3_1[,c("overallscore", "total_deaths", "no_injured", "no_affected", "no_homeless", "total_affected", "total_damages")], upper.panel=panel.cor, diag.panel=panel.hist, main="Correlation table and distribution of disasters variables")
```

We chose to replace the missing values of this dataset by zeros because we made the hypothesis that it's normal that disasters are not going to happen every year and in the same areas. So when we replace the NA by 0 we meant that at that moment any disaster did take place. The different variables used to materialize the impact of disasters do not seem to have important influence on the overall score, but we will further explore for the different SDGs, since we believe that disasters have a specific influence on some SDGs.
...
The latter Pearson's correlation coefficients method shows us the distribution of the goal scores, and again the positive and negative correlations between the goals. We notive again the same observations as we did in the previous correlation matrix heatmap. Now, we tried to use the same method for the Human Freedom Index scores.
...
panel.cor <- function(x, y, digits = 2, prefix = "", cex.cor, ...){
  ...
  txt <- format(c(r, 0.123456789), digits = digits)[1]
  txt <- paste0(prefix, txt)
  if(missing(cex.cor)) cex.cor <- 6/strwidth(txt)
  text(0.5, 0.5, txt, cex = cex.cor * r)
}
...
#3 revelant component according to eigenvalue of Kaiser-Guttman’s rule
...
# Results:  (between_SS / total_SS =  58.4 %)
```
